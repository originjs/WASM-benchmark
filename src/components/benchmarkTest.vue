<template>
  <div style="width: 70%; border: 1px solid; border-radius: 15px;">
    <div style="border-bottom: 1px solid;">
      <h3>Configure:</h3>
      <p>Warm-up run loops: <input style="width: 50px; text-align: center;" v-model="wasmTest.warmUpRunLoops"/></p>
      <p>Benchmark run loops: <input style="width: 50px; text-align: center;" v-model="wasmTest.benchmarkRunLoops"/></p>
      <p><button :disabled="btnDisabled" @click="start()" style="font-size: 15px;">Run Benchmark</button></p>
    </div>
    <br/>
    <div>
      <h3>Results:</h3>
      <p><span>Status : <code>{{ message }}</code></span></p>
      <p>JavaScript(average [ms]): <span>{{ jsPerformance }}</span></p>
      <p>WebAssembly(average [ms]): <span>{{ wsPerformance }}</span></p>
      <p>JavaScript/WebAssembly: <span>{{ comparison }}</span></p>
    </div>
  </div>
</template>
<script>
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            this.comparison = (this.jsPerformance / this.wsPerformance).toFixed(4);
            this.message = 'Done';
          });
